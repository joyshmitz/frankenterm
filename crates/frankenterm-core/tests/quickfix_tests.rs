...
use frankenterm_core::error_codes::{get_error_code, list_error_codes};
use frankenterm_core::event_templates::{EVENT_TEMPLATE_REGISTRY, render_event};
...
fn levenshtein_typo_detection_basic() {
    use frankenterm_core::suggestions::levenshtein_distance;
...
fn suggest_closest_finds_typo() {
    use frankenterm_core::suggestions::suggest_closest;
...
fn error_renderer_maps_all_error_variants_to_catalog() {
    use frankenterm_core::output::ErrorRenderer;
...
fn all_explanation_see_also_references_valid_targets() {
    for id in list_template_ids() {
        ...
        for reference in template.see_also {
            ...
            let valid = reference.starts_with("ft ")
            ...
            assert!(
                valid,
                "see_also for template '{id}' must be 'ft ...', 'wezterm ...', URL, or known tool, got: '{reference}'"
            );
        }
    }
}
...
fn error_renderer_plain_output_structure() {
    ...
    assert!(output.contains("ft why FT-"), "Must include 'ft why' hint");
}
...
fn error_code_format_plain_produces_stable_sections() {
    let def = get_error_code("FT-1001").expect("FT-1001 exists");
    let formatted = def.format_plain();

    assert!(formatted.contains("FT-1001"), "Should contain error code");
